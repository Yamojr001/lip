    ...
    <div className="flex h-screen bg-gray-100">
      ...
      >
        ...
        <div className="p-4 border-b border-purple-500">
            <p className="text-sm font-semibold">{t('common.welcome')},</p>
        ...
        <nav className="p-4 space-y-2 flex-1 overflow-y-auto">
          <NavItem href={route('phc.dashboard')} icon={LayoutDashboard} label={t('nav.dashboard')} />
          <NavItem href={route('phc.search')} icon={Search} label={t('nav.findPatient')} />
          <NavItem href={route('phc.create-patient')} icon={UserPlus} label={t('nav.registerPatient')} />
          <NavItem href={route('phc.children.index')} icon={Baby} label={t('nav.children')} />
          <NavItem href={route('phc.records')} icon={FileText} label={t('nav.myRecords')} />
          <NavItem href={route('phc.all-patients')} icon={ClipboardList} label={t('nav.allPatients')} />
          <NavItem href={route('phc.vaccine-accountability')} icon={Package} label={t('nav.vaccineAccountability')} />
          <NavItem href={route('phc.nutrition.reports.index')} icon={Apple} label={t('nav.nutritionReports')} />
          <NavItem href={route('phc.vaccine-reports.index')} icon={ClipboardList} label={t('nav.vaccineReports')} />
        </nav>
