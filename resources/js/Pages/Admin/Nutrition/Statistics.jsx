...
import { 
    ...
    Apple, BarChart3, RefreshCw, FileText, MapPin, Building
} from 'lucide-react';
...
export default function AdminNutritionStatistics() {
    ...
    if (monthlyData && typeof monthlyData === 'object') {
        Object.entries(monthlyData).forEach(([year, monthsData]) => {
    ...
    const statCards = [
        ...
        },
        {
            ...
            icon: Users,
            description: 'Pregnant women & caregivers'
        }
    ];
    
    const ageGroupData = [
        { name: '6-23 Months', male: 0, female: 0 },
        { name: '24-59 Months', male: 0, female: 0 }
    ];
    
    const interventionData = [
        { name: 'Albendazole', value: statistics.total_albendazole || 0, color: '#10b981' },
        { name: 'Vitamin A', value: statistics.total_vas || 0, color: '#ec4899' },
        { name: 'MNP', value: statistics.total_mnp_given || 0, color: '#8b5cf6' }
    ];
        ...
        <AdminLayout title="Nutrition Statistics Dashboard">
            <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Monthly Trends */}
                    <ChartCard title="Monthly Screening Trends">
                        <div className="h-80">
                    ...
                    <ChartCard title="Nutrition Status Distribution">
                        <div className="h-80">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie
                ...
                <ChartCard title="Interventions Distribution">
                    <div className="h-80">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                        ...
                        <table className="min-w-full text-sm text-left text-gray-600">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 font-semibold text-gray-700">Screened</th>
                                    <th className="px-4 py-3 font-semibold text-gray-700">SAM Rate</th>
                                    <th className="px-4 py-3 font-semibold text-gray-700">MAM Rate</th>
                                    <th className="px-4 py-3 font-semibold text-gray-700">RUTF Given</th>
                                </tr>
                            </thead>
                            <tbody>
                                {facilityPerformance.slice(0, 15).map((facility, index) => (
                                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        <td className="px-4 py-3">{facility.total_screened || 0}</td>
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded-full text-xs ${
                                                facility.avg_sam_rate >= 10 ? 'bg-red-100 text-red-800' :
                                                facility.avg_sam_rate >= 5 ? 'bg-yellow-100 text-yellow-800' :
                                                'bg-green-100 text-green-800'
                                            }`}>
                                                {facility.avg_sam_rate?.toFixed(1) || 0}%
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded-full text-xs ${
                                                facility.avg_mam_rate >= 15 ? 'bg-yellow-100 text-yellow-800' :
                                                facility.avg_mam_rate >= 10 ? 'bg-orange-100 text-orange-800' :
                                                'bg-green-100 text-green-800'
                                            }`}>
                                                {facility.avg_mam_rate?.toFixed(1) || 0}%
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">{facility.total_rutf || 0}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
