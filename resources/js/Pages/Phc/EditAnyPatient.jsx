...
import { CheckCircle, ArrowLeft, Save, Building, AlertCircle, User, MapPin, Stethoscope, Baby, Heart, FileText, Shield, Dna, DollarSign, FlaskConical, Pill as PillIcon, Syringe as SyringeIcon, Droplet, Microscope, Scale } from "lucide-react";
...
const DebouncedInput = React.memo(({ 
  ...
  return (
    <input
      ...
      required={required}
      disabled={disabled}
      className={`p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent outline-none transition text-base w-full ${
...
const DebouncedTextarea = React.memo(({ 
  ...
  return (
    <textarea
      ...
      rows={rows}
      disabled={disabled}
      className={`p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent outline-none transition text-base w-full resize-vertical ${
        disabled ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : ''
      } ${className}`}
      {...props}
    />
...
export default function EditAnyPatient() {
  const { patient, lgas = [], wards = [], phcFacilities = [], isCrossFacility = false } = usePage().props;
  ...
  const handleSubmit = (e) => {
    ...
    patch(route('phc.all-patients.update', patient.id), {
      onSuccess: () => {
  ...
  // Handle back to view (specific to cross-facility editing)
  ...
  const Checkbox = React.memo(({ label, name, checked, onChange, disabled = false }) => (
    <div className="flex items-center space-x-2">
        ...
        />
        <label htmlFor={name} className={`text-sm font-medium ${disabled ? 'text-gray-500' : 'text-gray-700'}`}>
            {label}
        </label>
    </div>
  ));
              ...
              {isCrossFacility && (
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-200 mb-4">
                    <p className="text-sm font-medium text-blue-800">
                        ...
                        Original facility preserved: {patient.health_facility?.clinic_name}
                    </p>
                </div>
              )}
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                <strong>Note:</strong> You are editing a patient originally registered at{' '}
                <strong>{patient.health_facility?.clinic_name}</strong>. The original registration facility will be preserved.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div className="flex items-center gap-4 mb-4 md:mb-0">
          ...
          <h1 className="text-2xl font-bold text-gray-800">
            Edit Patient Record
            {isCrossFacility && <span className="text-yellow-600 text-lg ml-2">(Cross-Facility)</span>}
          </h1>
        </div>
        <div className="bg-purple-100 px-4 py-2 rounded-lg">
          ...
          {isCrossFacility && (
            <div className="text-xs text-purple-600 mt-1">
              Original Facility: {patient.health_facility?.clinic_name}
            </div>
          )}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto pb-8">
        ...
        <InputSection title="Personal Information" icon={User}>
            ...
            <div>
              ...
              {isCrossFacility && <p className="text-xs text-gray-500 mt-1">LGA cannot be changed for cross-facility patients.</p>}
            </div>
            ...
            <div>
              ...
              {isCrossFacility && <p className="text-xs text-gray-500 mt-1">Ward cannot be changed for cross-facility patients.</p>}
            </div>
            
            <div>
              <label htmlFor="health_facility_id" className="block text-sm font-medium text-gray-700 mb-1">Health Facility (Current Site) *</label>
              <select 
            ...
            {isCrossFacility && (
              <div className="md:col-span-2 bg-blue-50 p-3 rounded-lg border border-blue-200">
                <div className="flex items-center gap-2 text-blue-800">
                  <Building size={16} />
                  <span className="font-semibold">Original Registration Facility:</span>
                  <span>{patient.health_facility?.clinic_name}</span>
                </div>
                <p className="text-xs text-blue-600 mt-1">
                  This patient was originally registered at the above facility and cannot be transferred.
                </p>
              </div>
            )}
        </InputSection>
